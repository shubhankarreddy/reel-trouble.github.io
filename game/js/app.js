...
function TopBar({ caseData, progress, onOpenCases }) {
...
function MobileWorkspaceNav({ activePanel, onChange, hasResult }) {
  return React.createElement('div', { className: 'mobile-workspace-nav' },
    React.createElement('button', {
      className: `mobile-workspace-btn ${activePanel === 'task' ? 'active' : ''}`,
      onClick: () => onChange('task')
    }, 'Task'),
    React.createElement('button', {
      className: `mobile-workspace-btn ${activePanel === 'editor' ? 'active' : ''}`,
      onClick: () => onChange('editor')
    }, hasResult ? 'Editor + Results' : 'Editor')
  );
}
...
function App() {
  ...
  useEffect(() => {
    const isMobileViewport = window.matchMedia && window.matchMedia('(max-width: 820px)').matches;
    if (view === 'playing' && textareaRef.current && (!isMobileViewport || mobilePanel === 'editor')) {
      textareaRef.current.focus();
    }
  }, [activeQuestionIdx, mobilePanel, view]);
  ...
  // Playing view
  return React.createElement('div', { id: 'game', className: `mobile-panel-${mobilePanel}` },
    React.createElement(TopBar, {
      caseData: currentCase,
      progress,
      onOpenCases: handleOpenCases
    }),
    React.createElement(MobileWorkspaceNav, {
